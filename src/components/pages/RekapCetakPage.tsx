...
export function RekapCetakPage({ showToast }: { showToast: (msg: string, type?: any) => void }) {
  ...
  const PDF_BTNS: { type: 'skl'|'dkn'|'nilaiIjazah'|'ijazah'|'transkrip'|'sk_kelulusan'|'skkb'; label: string; info: string }[] = [
    { type: 'skl',          label: 'Cetak SKL',           info: 'Surat Keterangan Lulus per siswa (A4 portrait). Nomor SKL dari biodata siswa.' },
    { type: 'dkn',          label: 'Cetak DKN',           info: 'Daftar Kumpulan Nilai semua siswa (A4 landscape). Berisi nilai per mapel dan rata-rata Nilai Ijazah.' },
    { type: 'nilaiIjazah',  label: 'Cetak Nilai Ijazah',  info: 'Daftar Nilai Ijazah per siswa (A4 portrait). Berisi kolom: Mapel, Rata Rapor, Nilai US, Nilai Ijazah.' },
    ...
    { type: 'transkrip',    label: 'Cetak Transkrip Nilai', info: 'Transkrip Nilai resmi per siswa (A4 portrait). Berisi biodata, tabel nilai akhir per mapel, dan TTD kepala sekolah.' },
    { type: 'sk_kelulusan', label: 'Cetak SK Kelulusan',  info: 'Surat Keputusan Kelulusan resmi (A4). Berisi halaman SK + lampiran daftar siswa lulus.' },
    { type: 'skkb',         label: 'Cetak SKKB',          info: 'Surat Keterangan Kelakuan Baik per siswa (A4 portrait). Nomor dari Penomoran Surat.' },
  ]
